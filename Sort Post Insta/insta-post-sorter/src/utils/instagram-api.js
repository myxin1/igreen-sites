(function attachInstagramApi(global) {
  ...
  function mapFeedItem(item) {
    ...
    const imageUrl = getFeedThumbnailUrl(item);
    const videoUrl = getFeedVideoUrl(item);
  ...
  function getFeedThumbnailUrl(item) {
    return getCandidateUrl(item.image_versions2 && item.image_versions2.candidates) ||
      item.thumbnail_url ||
      item.display_url ||
      item.preview_url ||
      getCandidateUrl(item.carousel_media && item.carousel_media[0] && item.carousel_media[0].image_versions2 && item.carousel_media[0].image_versions2.candidates) ||
      "";
  }

  function getFeedVideoUrl(item) {
    return getCandidateUrl(item.video_versions) ||
      item.video_url ||
      "";
  }

  function getCandidateUrl(candidates) {
    if (!Array.isArray(candidates)) return "";
    const candidate = candidates.find((entry) => entry && entry.url);
