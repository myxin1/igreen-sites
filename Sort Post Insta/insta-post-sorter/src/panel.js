(function createInstaPostSorterPanel(global) {
  ...
  function initUI(host, shadow) {
    ...
    function renderPostRow(post, index) {
      const thumb = getPostPreviewMarkup(post);
    ...
    function getPostPreviewMarkup(post) {
      if (post.thumbnailUrl) {
        return `<img class="result-thumb" src="${escAttr(post.thumbnailUrl)}" alt="">`;
      }
      if (post.videoUrl) {
        return `<video class="result-thumb" src="${escAttr(post.videoUrl)}" muted preload="metadata" playsinline></video>`;
      }
      return `<div class="result-thumb"></div>`;
    }
